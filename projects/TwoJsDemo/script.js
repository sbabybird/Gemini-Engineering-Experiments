document.addEventListener('DOMContentLoaded', () => {
    ...
    clearButton.addEventListener('click', () => { 
        mainLayer.remove(mainLayer.children); 
        deselectAll();
    });
    ...
    function onMouseDown(e) {
        ...
        if (state.activeTool === 'select') {
            ...
            if (hit) {
                if (!state.selection.includes(hit)) select(hit);
    ...
    function onMouseMove(e) {
        ...
        if (state.isPanning) {
            ...
            switch (state.activeTool) {
                case 'polyline':
                case 'polygon': handlePathMove(mouseWorld); break;
    ...
    function onMouseUp(e) {
        ...
        if (state.activeTool === 'polyline' || state.activeTool === 'polygon') {
            const mouseWorld = screenToWorld(new Two.Vector(e.clientX, e.clientY));
            if (state.drawStart.distanceTo(mouseWorld) < 2 / two.scene.scale) { // It's a click
                handlePathClick(mouseWorld);
            }
        } else if (state.isDrawing) {
    ...
    function onToolChange(e) { e.preventDefault(); finalizePath(); const target = e.currentTarget; state.activeTool = target.dataset.tool; toolButtons.forEach(b => b.classList.replace('contrast', 'secondary')); target.classList.replace('secondary', 'contrast'); deselectAll(); }
    ...
    function handlePathClick(mouseWorld) {
        if (!state.isDrawing) {
            state.isDrawing = true;
            const path = two.makePath(mouseWorld.x, mouseWorld.y, mouseWorld.x, mouseWorld.y);
    ...
    function finalizePath() {
        ...
        // A real shape must have at least 2 vertices (e.g. a line).
        // The preview shape has a "rubber-band" vertex, so we need at least 3 vertices total.
        if (state.previewShape.vertices.length < 3) {
            cancelPath();
            return;
        }

        state.previewShape.vertices.pop(); // Remove the rubber-band vertex

        if (state.activeTool === 'polygon') {
            state.previewShape.closed = true;
            applyCurrentStyle(state.previewShape); // Re-apply style to handle fill
        }
        state.previewShape = null; 
        state.isDrawing = false;
    }

    function cancelPath() { if (state.previewShape) { mainLayer.remove(state.previewShape); state.previewShape = null; state.isDrawing = false; } }
    ...
    function applyCurrentStyle(shape, overrides = {}) {
        shape.stroke = state.strokeColor || '#00aeff';
        shape.linewidth = (state.strokeWidth || 5) / two.scene.scale;
        const useFill = overrides.useFill !== undefined ? overrides.useFill : state.useFill;
        if (useFill) { shape.fill = state.fillColor || '#5c6a7e'; } else { shape.noFill(); }
    }
